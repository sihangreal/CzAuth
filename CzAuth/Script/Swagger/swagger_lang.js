...
var SwaggerTranslator = (function () {
    ...
    return {
        Translator: function () {
            ...
            $("#logo").html("开发SDK").attr("href", "/Home/Index");
            //设置控制器描述
...
setTimeout(function () {
    ...
    $.ajax({
        url: "http://" + location.host + "/api/auth/login",
        type: 'post',
        data: postdata,
        success: function (a) {
            if (a && a.isAuthencated) {
                $('#input_apiKey').val("");
                $('#input_apiKey').val("Bearer  " + a.accessToken);
            }
        }
    })
}, 3000);